...
export const fakeOrders = {
  ...
  async getAll({
    ...
    if (categories.length > 0) {
      products = products.filter((product) =>
        categories.includes(product.status)
      );
    }
  ...
  async getOrders({
    ...
    const categoriesArray = status ? status.split('.') : [];
    const allProducts = await this.getAll({
      categories: categoriesArray,
      search
    });
    ...
    // Pagination logic
    const offset = (page - 1) * limit;
    const paginatedOrders = allProducts.slice(offset, offset + limit);
